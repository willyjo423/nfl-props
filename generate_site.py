...
<style>
  ...
  body {{ font-family: -apple-system, Segoe UI, Roboto, sans-serif; max-width: 1100px; margin: 0 auto; padding: 24px 16px; background: #0f1115; color: #e7e9ee; }}
  ...
  th, td {{ text-align: left; padding: 8px 10px; border-bottom: 1px solid #22252d; }}
...
<body>
  ...
  <div class="disclaimer">
    ...
    position recently. No injury reports, depth-chart news, weather, or
    sportsbook lines are factored in. Treat this as a research starting
...
POSITION_SECTION = """
  <h2>{position}</h2>
  <table>
    <thead><tr><th>Player</th><th>Team</th><th>Opp</th>{stat_headers}<th>Sample</th></tr></thead>
    <tbody>
      {rows}
    </tbody>
  </table>
...
def render_position_section(position: str, players: list) -> str:
    ...
    rows = []
    for p in players:
        cells = [f"<td>{p['player']}</td>", f"<td>{p['team']}</td>", f"<td>@{p['opponent']}</td>"]
